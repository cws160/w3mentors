        ...
        <div class="card-class__head">
            <div class="card-class__media ratio ratio--16by9">
                <a href="<?php echo MyUtility::makeUrl('GroupClasses', 'view', [$class['grpcls_slug']]); ?>">
                    <img src="<?php echo FatCache::getCachedUrl(MyUtility::makeUrl('Image', 'show', [Afile::TYPE_GROUP_CLASS_BANNER, $class['grpcls_id'], Afile::SIZE_MEDIUM]), CONF_DEF_CACHE_TIME); ?>" alt="<?php echo $class['grpcls_title']; ?>" />
                </a>
            </div>
            <div class="card-class__action">
                <?php if ($class['grpcls_already_booked']) { ?>
                <button title="<?php echo Label::getLabel('LBL_ALREADY_BOOKED') ?>" tabindex="0" class="btn btn--white btn--disabled"><?php echo Label::getLabel("LBL_BOOK_NOW") ?></button>
                <?php } elseif ($class['grpcls_booked_seats'] >= $class['grpcls_total_seats']) { ?>
                <button title="<?php echo Label::getLabel('LBL_CLASS_FULL'); ?>" tabindex="0" class="btn btn--white btn--disabled"><?php echo Label::getLabel("LBL_BOOK_NOW") ?></button>
                <?php } elseif ($class['grpcls_start_datetime'] < date('Y-m-d H:i:s', strtotime('+' . $bookingBefore . ' minutes', $class['grpcls_currenttime_unix']))) { ?>
                <button title="<?php echo Label::getLabel('LBL_BOOKING_CLOSED') ?>" class="btn btn--white btn--disabled"><?php echo Label::getLabel("LBL_BOOK_NOW") ?></button>
                <?php } elseif ($siteUserId == $class['grpcls_teacher_id']) { ?>
                <button title="<?php echo Label::getLabel('LBL_CANNOT_BOOK_OWN_CLASS'); ?>" class="btn btn--white btn--disabled"><?php echo Label::getLabel("LBL_BOOK_NOW") ?></button>
                <?php } elseif ($class['grpcls_booked_seats'] + $class['grpcls_unpaid_seats'] >= $class['grpcls_total_seats']) { ?>
                <button title="<?php echo Label::getLabel('LBL_CLASS_HOLD_INFO'); ?>" class="btn btn--white btn--disabled"><?php echo Label::getLabel("LBL_BOOK_NOW") ?></button>
                <?php } elseif ($class['grpcls_type'] == GroupClass::TYPE_PACKAGE) { ?>
                <button onclick="cart.addPackage(<?php echo $class['grpcls_id']; ?>)" class="btn btn--white"><?php echo Label::getLabel("LBL_BOOK_NOW") ?></button>
                <?php } else { ?>
        ...
        <div class="card-class__body">
            <div class="card-flex-group">
                <div class="card-flex-group__item">
                    <span class="card-class__date"><?php echo MyDate::showDate($class['grpcls_start_datetime']); ?></span>
                </div>
            ...
            <h4 class="bold-700 price-value"><?php echo MyUtility::formatMoney($class['grpcls_entry_fee']); ?></h4>
        </div>
        <div class="card-class__footer">
            <?php
            $url = 'javascript:void(0);';
            if ($class['is_profile_complete'] == true) {
                $url = MyUtility::makeUrl('teachers', 'view', [$class['user_username']]);
            }
            ?>
            <a href="<?php echo $url; ?>" class="profile-meta d-flex align-items-center">
                ...
                <div class="profile-meta__details">
                    <h6 class="bold-600 profile-meta__title"><?php echo $class['user_full_name'] ?></h6>
                    <div class="rating">
