...
export function AdminSpeakLanguageModal({ open, slangId, onClose, onSaved }: Props) {
  const { lbl } = useSite();
  ...
  useEffect(() => {
    ...
    if (slangId > 0) {
      setRecordId(slangId);
      void adminApi.speakLanguageShow(slangId).then((res) => {
        const data = res.data.data as LanguageDetail;
        setIdentifier(data.slang_identifier ?? '');
        setStatus(String(data.slang_active ?? 1));
      });
    } else {
      reset();
    }
  ...
  const loadLangTab = useCallback((id: number, langId: number) => {
    ...
    setError('');
    void adminApi
      .speakLanguageShow(id, langId)
      .then((res) => {
        const data = res.data.data as LanguageDetail;
        setName(data.slang_name ?? '');
        setAutoTranslate(false);
      })
      .catch((err: unknown) => {
        setError(
          (err as { response?: { data?: { message?: string } } })?.response?.data?.message ??
            'Failed to load spoken language',
        );
      })
      .finally(() => setLoading(false));
  }, []);
  ...
  const onSaveGeneral = async (e: FormEvent) => {
    ...
    try {
      const payload = {
        slang_identifier: identifier.trim(),
        slang_active: Number(status),
        lang_id: 1,
      };
      const res =
        recordId > 0
          ? await adminApi.updateSpeakLanguage(recordId, payload)
          : await adminApi.createSpeakLanguage(payload);
      const savedId = Number(res.data?.data?.slang_id ?? recordId);
      setRecordId(savedId);
      onSaved();
      ...
      setError(
        (err as { response?: { data?: { message?: string } } })?.response?.data?.message ??
          'Failed to save spoken language',
      );
    } finally {
      setSaving(false);
    }
  ...
  const onSaveLang = async (e: FormEvent) => {
    ...
    setError('');
    try {
      await adminApi.updateSpeakLanguage(recordId, {
        slang_identifier: identifier.trim(),
        slang_name: name.trim(),
        slang_active: Number(status),
        lang_id: langId,
  ...
  return (
    <AdminModal
      open={open}
      title={lbl('LBL_SPOKEN_LANGUAGE_SETUP', 'Spoken language setup')}
      size="md"
      onClose={onClose}
    >
            ...
            <div className="row">
              <div className="col-md-12">
                <div className="field-set">
                  <div className="caption-wraper">
                    <label className="field_label">
                      {isArabicLang ? 'اسم' : lbl('LBL_SPEAK_LANGUAGE_NAME', 'Name')}
                      <span className="spn_must_field">*</span>
                    </label>
                  </div>
                  <div className="field-wraper">
