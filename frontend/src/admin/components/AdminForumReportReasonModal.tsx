...
type ReasonDetail = {
  frireason_id: number;
  frireason_identifier: string;
  frireason_name: string;
  frireason_active: number;
};
...
export function AdminForumReportReasonModal({ open, reasonId, onClose, onSaved }: Props) {
  const { lbl } = useSite();
  ...
  useEffect(() => {
    ...
    if (reasonId > 0) {
      setRecordId(reasonId);
      void adminApi.forumReportReasonShow(reasonId).then((res) => {
        const data = res.data.data as ReasonDetail;
        setIdentifier(data.frireason_identifier ?? '');
        setStatus(String(data.frireason_active ?? 1));
      });
    } else {
      reset();
    }
  ...
  const loadLangTab = useCallback((id: number, langId: number) => {
    ...
    setError('');
    void adminApi
      .forumReportReasonShow(id, langId)
      .then((res) => {
        const data = res.data.data as ReasonDetail;
        setTitle(data.frireason_name ?? '');
        setAutoTranslate(false);
      })
      .catch((err: unknown) => {
        setError(
          (err as { response?: { data?: { message?: string } } })?.response?.data?.message ??
            'Failed to load report reason',
        );
      })
      .finally(() => setLoading(false));
  }, []);
  ...
  const onSaveGeneral = async (e: FormEvent) => {
    ...
    try {
      const payload = {
        frireason_identifier: identifier.trim(),
        frireason_active: Number(status),
        lang_id: 1,
      };
      const res =
        recordId > 0
          ? await adminApi.updateForumReportReason(recordId, payload)
          : await adminApi.createForumReportReason(payload);
      const savedId = Number(res.data?.data?.frireason_id ?? res.data?.data?.id ?? recordId);
      setRecordId(savedId);
      onSaved();
  ...
  const onSaveLang = async (e: FormEvent) => {
    ...
    setError('');
    try {
      await adminApi.updateForumReportReason(recordId, {
        frireason_identifier: identifier.trim(),
        frireason_name: title.trim(),
        frireason_active: Number(status),
        lang_id: langId,
  ...
  return (
    <AdminModal
      open={open}
      title={lbl('LBL_REPORT_REASON_SETUP', 'Report reason setup')}
      size="md"
      onClose={onClose}
    >
