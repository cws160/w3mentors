...
import { AdminSpeakLanguageLevelModal } from '../components/AdminSpeakLanguageLevelModal';
...
export function AdminSpeakLanguageLevelsPage() {
  ...
  const canEdit = Boolean(privileges.canEditSpeakLanguageLevels) || admin?.id === 1;

  const load = useCallback(() => {
    setLoading(true);
    void adminApi
      .moduleList('speak-language-levels', { lang_id: langId })
      .then((res) => {
  ...
  useEffect(() => {
    let cancelled = false;
    void adminApi.pageText('spoken-language-levels').then((res) => {
      if (cancelled) return;
      const pageText = res.data.data ?? {};
      setMeta({
        title: pageText.title || lbl('LBL_SPOKEN_LANGUAGE_LEVELS', 'Spoken language levels'),
  ...
  const onExport = () => {
    const header = [
      ...
      lbl('LBL_LANGUAGE_LEVEL_NAME', 'Name'),
    ...
    link.download = 'spoken-language-levels.csv';
    link.click();
  ...
  const onDelete = async (id: number) => {
    ...
    try {
      await adminApi.deleteSpeakLanguageLevel(id);
      load();
    } catch (err: unknown) {
      const message =
  ...
  const reorder = async (nextRows: Row[]) => {
    setRows(nextRows);
    try {
      await adminApi.updateSpeakLanguageLevelOrder(nextRows.map((row) => row.id));
      load();
    } catch {
      load();
    }
      ...
      <div className="container">
        <div className="breadcrumb-wrap">
          <ul className="breadcrumb">
            ...
            <li className="breadcrumb-item">
              {lbl('LBL_SPOKEN_LANGUAGE_LEVELS', 'Spoken language levels')}
            </li>
          </ul>
          ...
          <div className="card-table">
            <div className="table-responsive" id="listing">
              {loading ? (
                ...
                <table className="table table--hovered table-dragable" width="100%" id="spokenLanguageLevels">
                  <thead>
                    <tr>
                      {canEdit ? (
                        <th style={{ width: '5%' }}>
                      ...
                      <th>{lbl('LBL_LANGUAGE_LEVEL_IDENTIFIER', 'Identifier')}</th>
                      <th>{lbl('LBL_LANGUAGE_LEVEL_NAME', 'Name')}</th>
                        ...
                        >
                          ...
                          <td>
                            <AdminStatusSwitch
                              ...
                              onToggle={async (next) => {
                                await adminApi.updateSpeakLanguageLevelStatus(row.id, next);
                                load();
                              }}
                            />
                          </td>
