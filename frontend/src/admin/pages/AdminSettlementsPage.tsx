...
type Row = {
  id: string;
  slstat_date: string;
  slstat_refund: string;
  slstat_earnings: string;
  slstat_teacher_paid: string;
};

const searchConfig: AdminModuleConfig = {
  module: 'settlements',
  pageLangKey: 'settlements',
  titleKey: 'LBL_SETTLEMENTS',
...
export function AdminSettlementsPage() {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        tooltip: lbl('ASR_REFUND_TOOLTIP', 'Refund tooltip'),
      },
      {
        key: 'slstat_earnings',
        label: lbl('LBL_EARNINGS', 'Earnings'),
        tooltip: lbl('ASR_EARNINGS_TOOLTIP', 'Earnings tooltip'),
      },
      {
        key: 'slstat_teacher_paid',
        label: lbl('LBL_TEACHER_PAID', 'Teacher paid'),
        tooltip: lbl('ASR_TEACHER_PAID_TOOLTIP', 'Teacher paid tooltip'),
      },
    ],
    [lbl],
  );
  ...
  const load = useCallback(() => {
    setLoading(true);
    void adminApi
      .moduleList('settlements', { page, ...filters })
      .then((res) => {
  ...
  useEffect(() => {
    void adminApi.pageText('settlements').then((res) => {
      const pageText = res.data.data ?? {};
      setMeta({
        title: pageText.title || lbl('LBL_SETTLEMENTS', 'Settlements'),
      ...
      <div className="container">
        ...
        <div className="breadcrumb-wrap">
          <ul className="breadcrumb">
            ...
            <li className="breadcrumb-item">{lbl('LBL_SETTLEMENTS', 'Settlements')}</li>
          </ul>
