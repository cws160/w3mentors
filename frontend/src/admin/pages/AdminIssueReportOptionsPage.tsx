...
import { AdminIssueReportOptionModal } from '../components/AdminIssueReportOptionModal';
...
export function AdminIssueReportOptionsPage() {
  ...
  const canEdit = Boolean(privileges.canEditIssueReportOptions) || admin?.id === 1;

  const load = useCallback(() => {
    setLoading(true);
    void adminApi
      .moduleList('issue-report-options', { lang_id: langId })
      .then((res) => {
  ...
  useEffect(() => {
    setMeta({
      title: lbl('LBL_ISSUE_REPORT_OPTIONS', 'Issue report options'),
    });

    let cancelled = false;
    void adminApi.pageText('issue-report-options').then((res) => {
      if (cancelled) return;
      const pageText = res.data.data ?? {};
      setMeta({
        title: pageText.title || lbl('LBL_ISSUE_REPORT_OPTIONS', 'Issue report options'),
  ...
  const onExport = () => {
    ...
    link.download = 'issue-report-options.csv';
    link.click();
  ...
  const onDelete = async (id: number) => {
    ...
    try {
      await adminApi.deleteIssueReportOption(id);
      load();
    } catch (err: unknown) {
      const message =
  ...
  const reorder = async (nextRows: Row[]) => {
    setRows(nextRows);
    try {
      await adminApi.updateIssueReportOptionOrder(nextRows.map((row) => row.id));
      load();
    } catch {
      load();
    }
      ...
      <div className="container">
        <div className="breadcrumb-wrap">
          <ul className="breadcrumb">
            ...
            <li className="breadcrumb-item">{lbl('LBL_ISSUE_REPORT_OPTIONS', 'Issue report options')}</li>
          </ul>
          ...
          <div className="card-table">
            <div className="table-responsive" id="listing">
              {loading ? (
                ...
                <table className="table table--hovered table-dragable" width="100%" id="IssueReportOptions">
                  <thead>
                    <tr>
                      {canEdit ? (
                        <th style={{ width: '5%' }}>
                        ...
                        >
                          ...
                          <td>
                            <AdminStatusSwitch
                              ...
                              onToggle={async (next) => {
                                await adminApi.updateIssueReportOptionStatus(row.id, next);
                                load();
                              }}
                            />
                          </td>
