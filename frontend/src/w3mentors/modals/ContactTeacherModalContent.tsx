...
export function ContactTeacherModalContent({ teacherId, teacherName, onSent }: Props) {
  const { lbl } = useSite();
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      const res = await chatsApi.privateThread(teacherId, text);
    ...
    <>
      <div className="modal-header">
        <h4 className="flex-1 text-center">
          {lbl('LBL_CONTACT', 'Contact')} {teacherName}
        </h4>
        <button type="button" className="btn-close" aria-label="Close" onClick={closeModal} />
      </div>
      <div className="modal-body">
        <div className="form-group">
          <label className="form-label">{lbl('LBL_MESSAGE', 'Message')}</label>
          <textarea
