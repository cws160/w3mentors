...
use App\Services\Admin\AdminCourseRefundRequestService;
use App\Services\Admin\AdminPrivilegeService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use RuntimeException;

class AdminCourseRefundRequestsController extends Controller
{
    public function __construct(
        private AdminPrivilegeService $privileges,
        private AdminCourseRefundRequestService $requests,
    ) {
    }
    ...
    public function show(Request $request, int $requestId): JsonResponse
    {
        return $this->guardView($request, function () use ($requestId) {
            try {
                ...
                return response()->json(['message' => 'Failed to load request'], 500);
            }
    ...
    {
        return $this->guardView($request, function () use ($requestId) {
            $data = $this->requests->statusForm($requestId);
            if (! $data) {
                return response()->json(['message' => 'Record not found'], 404);
            }
    ...
    private function guardView(Request $request, callable $callback): JsonResponse
    {
        ...
        if (! $this->privileges->canView($adminId, AdminPrivilegeService::SECTION_COURSE_REFUND_REQUESTS)) {
    ...
    {
        $adminId = $this->adminId($request);
        if (! $this->privileges->canEdit($adminId, AdminPrivilegeService::SECTION_COURSE_REFUND_REQUESTS)) {
            return response()->json(['message' => 'Unauthorized'], 403);
        }
