...
class AdminForumTagRequestsController extends Controller
{
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        return $this->guardView($request, function () use ($id) {
            $data = $this->forum->tagRequestShow($id);
            if ($data === null) {
    ...
    public function changeStatus(Request $request, int $id): JsonResponse
    {
        return $this->guardEdit($request, function () use ($request, $id) {
            try {
                $this->forum->tagRequestChangeStatus($id, (int) $request->input('ftagreq_status'));
