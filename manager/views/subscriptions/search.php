    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            case 'action':
                ...
                $li->appendElement('a', ['href' => MyUtility::makeUrl('Orders', 'viewInvoice', [$row['order_id']]).'?t='.time(), 'class' => 'button small green', 'title' => Label::getLabel('LBL_DOWNLOAD_INVOICE'), 'target' => '_blank'], '<svg class="svg" width="18" height="18"><use xlink:href="' . CONF_WEBROOT_BACKEND . 'images/retina/sprite-actions.svg#download"></use></svg>', true);
                break;
            case 'order_id':
