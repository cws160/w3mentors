    ...
    foreach ($arrFlds as $key => $val) {
        ...
        switch ($key) {
            ...
            case 'user_name':
                ...
                break;
            case 'coedre_status':
                $td->appendElement('plaintext', [], CommonHelper::renderHtml($requestStatus[$row['coedre_status']]));
                break;
            case 'coedre_created':
            ...
            case 'course_title':
                ...
                break;
            case 'action':
                $action = new Action($row['coedre_id']);
                $action->addViewBtn(Label::getLabel('LBL_VIEW'),  'view(' . $row['coedre_id'] . ')');
                if ($canEdit && $row['coedre_status'] == Course::EDIT_REQUEST_PENDING) {
                    $action->addOtherBtn(Label::getLabel('LBL_CHANGE_STATUS'), 'changeStatusForm("' . $row['coedre_id'] . '")', 'edit', 'javascript:void(0)');
                }
